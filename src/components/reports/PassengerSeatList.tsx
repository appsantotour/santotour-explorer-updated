...
import PrintablePassengerSeatListReport from './prints/PrintablePassengerSeatListReport';
...
interface PassengerSeatData extends Omit<PassageiroFormData, 'poltrona'> {
  poltrona: number | null; // Now aligned with Supabase INT4
}

const PassengerSeatList: React.FC = () => {
  ...
  const handleSaveChanges = async () => {
    if (!selectedTripId || passengers.length === 0) {
      toast({ title: "Nenhuma Alteração", description: "Selecione uma viagem e certifique-se de que há passageiros.", variant: "warning" });
      return;
    }
    setIsSaving(true);
    ...
    <div className="p-4 md:p-6 bg-gray-50 min-h-screen">
      ...
      {isPrinting && printAreaTarget && printableTripInfo &&
        ReactDOM.createPortal(
          <PrintablePassengerSeatListReport
...
export default PassengerSeatList;
